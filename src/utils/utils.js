...
class Utils {
  static login(Bot) {
    Bot.login(Config.TOKEN ? Config.TOKEN : process.env.TOKEN)
      .then(() => console.log("[BOT] iqo Ticket Bot Aktif."))
  ...
  static embed(Content, Guild, Bot, User) {
    const iqorenk = Config.TICKET.PanelEmbedMesajRenkKod
    const Embed = new EmbedBuilder()
      .setAuthor({
        name: `${Guild.name} Ticket Sistemi`,
  ...
  static modal() {
    ...
    let Modals = new ModalBuilder()
      .setCustomId("ticket")
      .setTitle("Ticket Oluşturma Talebi");
