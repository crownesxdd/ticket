  ...
  Bot.on("interactionCreate", async (interaction) => {
        ...
        if (Channel) {
          ...
          interaction.guild.channels
            ...
            .then(async (Channel) => {
              interaction.followUp({
                content:
                `Hey! Bilet talebiniz başarıyla oluşturuldu. ${Channel}`,
                ephemeral: true,
              });
    ...
    if (interaction.customId === "successTicket") {
      if (
        ...
        interaction.followUp({
          content: `Bilet doğrulama sistemini sadece yetkililer kullanabilir.`,
          ephemeral: true,
        });
        ...
        interaction.channel.send({
          content: `Heyy! <@!${interaction.channel.name.replace(
            "ticket-",
            ""
          )}>, biletin yetkililer tarafından başarıyla onaylandı.`,
        });
    ...
    if (interaction.customId === "deleteTicket") {
      ...
      interaction.followUp({
        content: `İsteğiniz başarıyla alındı, \`5 saniye\` sonra kanal otomatik olarak silinecektir.`,
        ephemeral: true,
      });
